...
var require_k_bucket = __commonJS({
  "../../node_modules/k-bucket/index.js"(exports, module2) {
    ...
    var KBucket2 = class extends EventEmitter2 {
      /**
       * `options`:
       *   `distance`: _Function_
      ...
      constructor(options = {}) {
        ...
        this.distance = options.distance || KBucket2.distance;
        this.arbiter = options.arbiter || KBucket2.arbiter;
...
__publicField(Pkarr, "generateKeyPair", generateKeyPair);
__publicField(Pkarr, "generateSeed", randomBytes);
  ...
  static toDnsPacket({ didDocument }) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (didDocument.authentication) {
        const authIds = didDocument.authentication.map((id) => DidDht.identifierFragment({ identifier: id })).filter((id) => keyLookup.has(id)).map((id) => keyLookup.get(id));
        if (authIds.length) {
          rootRecord.push(`auth=${authIds.join(",")}`);
        }
      }
      if (didDocument.assertionMethod) {
        const authIds = didDocument.assertionMethod.map((id) => DidDht.identifierFragment({ identifier: id })).filter((id) => keyLookup.has(id)).map((id) => keyLookup.get(id));
        if (authIds.length) {
          rootRecord.push(`asm=${authIds.join(",")}`);
        }
      }
      if (didDocument.keyAgreement) {
        const authIds = didDocument.keyAgreement.map((id) => DidDht.identifierFragment({ identifier: id })).filter((id) => keyLookup.has(id)).map((id) => keyLookup.get(id));
        if (authIds.length) {
          rootRecord.push(`agm=${authIds.join(",")}`);
        }
      }
      if (didDocument.capabilityInvocation) {
        const authIds = didDocument.capabilityInvocation.map((id) => DidDht.identifierFragment({ identifier: id })).filter((id) => keyLookup.has(id)).map((id) => keyLookup.get(id));
        if (authIds.length) {
          rootRecord.push(`inv=${authIds.join(",")}`);
        }
      }
      if (didDocument.capabilityDelegation) {
        const authIds = didDocument.capabilityDelegation.map((id) => DidDht.identifierFragment({ identifier: id })).filter((id) => keyLookup.has(id)).map((id) => keyLookup.get(id));
        if (authIds.length) {
